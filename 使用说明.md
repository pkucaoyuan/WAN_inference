...
**🔬 三维评分 + 量级对齐优势**：
- **变化幅度 (基准量级)**: 作为基准量级，保持原始尺度
- **自注意力 (缩放对齐)**: 缩放到变化分数量级，保持相对重要性
- **跨模态注意力 (缩放对齐)**: 缩放到变化分数量级，平衡多模态信息

**⚙️ 关键可调参数**：
```python
baseline_steps = 5          # 基准期长度
percentile_threshold = 20   # 修剪阈值百分位数 (可调: 10-30)
change_weight = 0.4         # 变化分数权重
