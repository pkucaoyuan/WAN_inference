...
```python
# 综合评分机制
def calculate_composite_score(token_idx, current_hidden, prev_hidden, 
                            self_attn_weights, cross_attn_weights):
    # 1. Token变化分数（变化幅度）
    change_score = torch.norm(current_hidden - prev_hidden, dim=-1)
    
    # 2. 图像token自注意力分数
    # 该token与所有图像token的自注意力权重总和
    image_self_attn = self_attn_weights[0, :, token_idx, image_start:image_end]
    self_attn_score = image_self_attn.mean(dim=0).sum()
    
    # 3. 图像-文本跨模态注意力分数  
    # 该图像token对所有文本token的注意力权重总和
    cross_attn_score = cross_attn_weights[0, :, token_idx, :].mean(dim=0).sum()
    
    # 4. 加权综合评分
    composite_score = (
        change_weight * change_score +           # 0.4 × 变化幅度
        self_attn_weight * self_attn_score +     # 0.3 × 图像自注意力
        cross_attn_weight * cross_attn_score     # 0.3 × 跨模态注意力
    )
    
    return composite_score

# 应用修剪
for layer_idx, block in enumerate(high_noise_model.blocks):
    if layer_idx >= start_pruning_layer:
        # 计算所有图像token的综合评分
        token_scores = [calculate_composite_score(i, ...) for i in image_tokens]
        
        # 根据评分排序，保留top-k重要token
        keep_tokens = sorted_by_score[:min_active_tokens]
        prune_tokens = sorted_by_score[min_active_tokens:]
        
        # 选择性更新：只更新重要token，冻结不重要token
        active_mask = create_mask_from_keep_tokens(keep_tokens)
        hidden_states = selective_update(hidden_states, active_mask)
```

**💡 预期收益**：
- **智能修剪**: 基于3个维度的综合评分，更精确的token选择
- **计算减少**: 20-40%的token停止更新，专注重要token
- **专家分工优化**: 
  - 高噪声专家：专注高变化+高自注意力+高跨模态关联的结构token
  - 低噪声专家：处理被修剪的细节token和全局优化
- **质量保持**: 多维度评分确保重要token继续更新
- **内存优化**: 冻结token的KV缓存复用，减少重复计算

**🎯 三维评分优势**：
- **变化幅度**: 识别仍在演化的动态区域
- **自注意力**: 识别图像内部结构关键点
- **跨模态注意力**: 识别与文本语义强相关的区域
