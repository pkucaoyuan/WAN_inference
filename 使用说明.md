...
for layer_idx, block in enumerate(high_noise_model.blocks):
    if layer_idx >= start_pruning_layer:
        # 1. 计算token变化幅度
        token_changes = torch.norm(current_hidden - prev_hidden, dim=-1)
        
        # 2. 识别结构性token（高注意力权重）
        structural_tokens = identify_structural_tokens(attention_weights)
        
        # 3. 动态停止变化小的非结构token
        active_mask = (token_changes > threshold) | structural_tokens
        
        # 4. 保存冻结token的KV用于attention计算
        frozen_kv = save_frozen_token_kv(hidden_states, ~active_mask)
        
        # 5. 只更新激活token，保持冻结token不变
        hidden_states = selective_update(hidden_states, active_mask)
```

**💡 预期收益**：
- **计算减少**: 20-40%的token停止更新
- **专家分工**: 高噪声专家专注结构，低噪声专家处理细节
- **质量保持**: 结构token继续更新，保证整体质量
- **内存优化**: 冻结token的激活值可复用
